...
class PyramidFeatures(nn.Module):
    def __init__(self, C3_size, C4_size, C5_size, feature_size=256):
        ...
        self.P5_upsampled   = Interpolate(scale_factor=2, mode='nearest')
        self.P5_2           = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)

        # add P5 elementwise to C4
        self.P4_1           = nn.Conv2d(C4_size, feature_size, kernel_size=1, stride=1, padding=0)
        #self.P4_upsampled   = nn.Upsample(scale_factor=2, mode='nearest')
        self.P4_upsampled   = Interpolate(scale_factor=2, mode='nearest')
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, block, layers):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.fill_(1) # not mentioned
                m.bias.data.zero_()
        """
        # Initialize FPN
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            anchors_nms_idx, max_k = nms(torch.cat([transformed_anchors, scores], dim=2)[0, :, :], 0.5)
